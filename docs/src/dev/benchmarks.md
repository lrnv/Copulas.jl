...
An optional documentation build compares Copulas.jl with R's
...
The comparison measures the Julia operations directly and
...
startup are excluded. The resulting table is inserted when the `Documenter`
workflow is launched manually with its `benchmarks` option. Routine builds skip
these experiments: Tachometer tracks native performance changes, while
rerunning the Julia/R comparison on every prose change would add noisy runner
measurements and several minutes of CI time.
...
get(ENV, "COPULAS_DOCS_BENCHMARKS", "false") == "true" ? # hide
    ...
    Markdown.parse("Results are generated by a manual `Documenter` workflow run with the `benchmarks` option enabled.") # hide
```

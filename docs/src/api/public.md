...
| Fitting, inference, and selection | `fit`, `CopulaModel`, `infer`, `CopulaInference`, `selectiontable`, `fitting_methods`, and `fit_copula` | Documented family/method pairs return valid point estimates. `CopulaModel` retains fit evidence; `infer` applies a separate uncertainty procedure whose covariance belongs to `CopulaInference`. Automatic family selection compares an explicit collection of candidates, and the two extension functions form the supported downstream estimator protocol; see the [fitting interface](@ref fitting_interface). |

...
| Dependence | `τ`, `ρ`, `β`, `γ`, `ι`, `λₗ`, `λᵤ`, `StatsBase.corkendall`, `StatsBase.corspearman` | Results have the documented scalar or pairwise-matrix shape, bounds and symmetry. `ι(C)` is `-D_KL(P_C || λ^d)`: the usual negative density entropy for absolutely continuous copulas and `-Inf` whenever the copula law has a singular component. Closed forms and numerical fallbacks have the same contract. Parameter inversions used by fitting are internal. |

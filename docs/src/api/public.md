...
| Fitting and selection | `fit`, `CopulaModel`, `selectiontable` and the StatsBase model interface | Documented family/method pairs return valid fitted models. `CopulaModel` exposes observations, coefficients, covariance when computed, information criteria, residuals and prediction. Automatic family selection compares an explicit collection of candidates; see the [fitting interface](@ref fitting_interface). |

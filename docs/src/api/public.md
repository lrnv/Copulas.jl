...
| Fitting, inference, and selection | `fit`, `CopulaModel`, `fittingmethod`, `infer`, `CopulaInference`, `CopulaSelection`, `selectedmodel`, `selectiontable`, `fitting_methods`, and `fit_copula` | Documented family/method pairs return valid point estimates. `CopulaModel` retains only the fitted result, original data, fitted likelihood, and replay recipe; derived diagnostics are computed through accessors. `infer` owns uncertainty state, while `CopulaSelection` separately owns candidate comparisons. The two extension functions form the supported downstream estimator protocol; see the [fitting interface](@ref fitting_interface). |

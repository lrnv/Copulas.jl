...
def replace(path, old, new):
    text = path.read_text()
    if old not in text:
        raise RuntimeError(f"missing expected block in {path}: {old[:90]!r}")
...
def sub(path, pattern, repl, *, count=1):
    ...
    if n != count:
        raise RuntimeError(f"expected {count} regex replacement(s) in {path}, got {n}")
    path.write_text(updated)


# ---------------------------------------------------------------------------
# Fitting and inference consume only the Copulas-side parameter geometry API.
# ---------------------------------------------------------------------------
fitting = SRC / "Fitting.jl"
sub(
    fitting,
    r'''function _concrete_paramorph_type\(T::Type, ::Type\{N\}=Float64\) where \{N<:Real\}.*?function _fit_prototype\(CT::Type\{<:Copula\}, ::Val\{d\}\) where \{d\}.*?end\n\n''',
...
inference = SRC / "Inference.jl"
text = inference.read_text()
text = text.replace("Paramorph.unconstrain(fitted)", "_parameter_coordinates(fitted)")
text = text.replace("Paramorph.constraint(fitted, αv)", "_from_parameter_coordinates(fitted, αv)")
inference.write_text(text)

# ---------------------------------------------------------------------------
# Delete the 0.0.2 extension protocol from structural wrappers. Their geometry
# is expressed only in the adapter below.
# ---------------------------------------------------------------------------
sub(
    SRC / "SklarDist.jl",
    r'''Paramorph\.parameter_fields\(::Type\{<:SklarDist\}\).*?Paramorph\.reconstruct_struct\(S::SklarDist, values::NamedTuple\) = SklarDist\(.*?\n\)\n''',
    "",
)

sub(
    SRC / "ArchimedeanCopula.jl",
    r'''Paramorph\.parameter_fields\(::Type\{<:ArchimedeanCopula\}\).*?function Paramorph\.reconstruct_struct\(.*?\nend\n''',
    "",
)

sub(
    SRC / "ExtremeValueCopula.jl",
    r'''Paramorph\.parameter_fields\(::Type\{<:ExtremeValueCopula\}\).*?function Paramorph\.reconstruct_struct\(.*?\nend\n''',
    "",
)

sub(
    SRC / "ArchimaxCopula.jl",
    r'''Paramorph\.is_paramorph_type\(::Type\{<:ArchimaxCopula\}\).*?function Paramorph\.reconstruct_struct\(C::ArchimaxCopula\{d\}, values::NamedTuple\) where \{d\}.*?\nend\n''',
    "",
)

sub(
    SRC / "LiouvilleCopula.jl",
    r'''Paramorph\.is_paramorph_type\(::Type\{<:LiouvilleCopula\}\).*?function Paramorph\.reconstruct_struct\(C::LiouvilleCopula\{d\}, values::NamedTuple\) where \{d\}.*?\nend\n''',
    "",
)

sub(
    SRC / "MiscellaneousCopulas/SurvivalCopula.jl",
    r'''Paramorph\.is_paramorph_type\(::Type\{<:AbstractReflectedCopula\}\).*?Paramorph\.reconstruct_struct\(C::Rotated270Copula, values::NamedTuple\) =\n    Rotated270Copula\(Paramorph\.reconstruct_struct\(basecopula\(C\), values\)\)\n''',
    "",
)

# Leaf types that still used the old external hook protocol are temporarily
# handled by the centralized adapter. They are the next candidates for a DSL
# improvement once the main bridge is running.
sub(
    SRC / "Tail/HuslerReissTail.jl",
    r'''Paramorph\.is_paramorph_type\(::Type\{<:HuslerReissTail\}\).*?Paramorph\.reconstruct_struct\(::HuslerReissTail\{<:AbstractMatrix\}, values::NamedTuple\) =\n    HuslerReissTail\(values\.Γ\)\n''',
    "",
)
sub(
    SRC / "Tail/tEVTail.jl",
    r'''Paramorph\.is_paramorph_type\(::Type\{<:tEVTail\}\).*?Paramorph\.reconstruct_struct\(::tEVTail\{<:Any,<:AbstractMatrix\}, values::NamedTuple\) =\n    tEVTail\(values\.ν, values\.R\)\n''',
    "",
)

# ---------------------------------------------------------------------------
# The single Paramorph bridge. No model file should extend Paramorph methods.
# ---------------------------------------------------------------------------
bridge = SRC / "ParamorphFitting.jl"
bridge.write_text(r'''# Internal bridge between Copulas' notion of parameter geometry and Paramorph.
#
...
print("central Paramorph bridge refactor applied")

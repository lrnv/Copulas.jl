...
# ---------------------------------------------------------------------------------
# Table of cases (Taken from and identical to the canonical tests of MvNormalCDF.jl)
# ---------------------------------------------------------------------------------
td = Array{Any}(undef,(14,9))
#  1-cov mtx 2-a 3-b 4-m 5-p 6-ptol 7-e 8-etol

# from MATLAB documentation 4 dim
td[1,1] = [4 3 2 1;3 5 -1 1;2 -1 4 2;1 1 2 5]
td[1,2] = [-Inf; -Inf; -Inf; -Inf]
td[1,3] = [1; 2; 3; 4 ]
td[1,4] = 5000
td[1,5] = 0.605653
td[1,6] = 0.001557374
...
# Genz used wrong integration limits when computing above (p. 63)
td[8,1] = td[7,1]
td[8,2] = sort(td[7,2])
td[8,3] = 1 .- td[8,2]
td[8,4] = 6000
...
# ------------------------------------------------------------------
# Compatibility test for _cdf_base(Σ, x) with MvNormalCDF.jl
# ------------------------------------------------------------------
@testset "MvNormalCDF compatibility - _CDF_BASE" begin
    # Select rows where a == (-Inf,...,-Inf)
    idxs = Int[]
    for i in 1:size(td, 1)
        a = td[i,2]
        if a !== nothing
            a_vec = collect(a)
            if all(x -> x == -Inf, a_vec)
                push!(idxs, i)
            end
        end
    end
    ...
    for i in idxs
        Σ = Array{Float64}(td[i,1])
        b = collect(Float64, td[i,3])
        d = length(b)
        m = Int(td[i,4])
        p_expected = td[i,5]
        ptol       = td[i,6]

        # omit non-SPD cases (e.g., singular)
        if !isposdef(Symmetric(Σ))
            @info "Skipping non-SPD case (expected in singular examples)" i
            continue
        end

        rng_ref = StableRNG(1234 + i)
        rng_est = StableRNG(2234 + i)

        # Reference (MvNormalCDF)
        p_ref, e_ref = MvNormalCDF.mvnormcdf(Σ, fill(-Inf, d), b; m=m, rng=rng_ref)
        @test isapprox(p_ref, p_expected; atol=ptol) || @warn("Reference p_ref differs from p_expected", i, p_ref, p_expected, ptol)

        # Estimation with our method
        p_est = Copulas._cdf_base(Σ, b; m0=m, r=1, maxpts=m, abseps=0.0, releps=0.0, rng=rng_est)

        # Cross-tolerance: adaptive by dimension and variance
        tol_cross = max(1e-6, 20*e_ref, 2*ptol)
        ok = isapprox(p_est, p_ref; atol=tol_cross) ||
            isapprox(p_est, p_expected; atol=tol_cross)
        if !ok
            Δ = abs(p_est - p_ref)
            relerr = Δ / p_ref
            @warn "p_est out of tolerance" i d m p_est p_ref p_expected tol_cross Δ relerr
        end
        @test ok
    end
end

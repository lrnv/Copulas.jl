...
#=
## ------------------------------------------------------------
## R References for t-Copula using 'copula' (only a=-Inf cases)
## Cases: i ∈ {1,2,4,5,6,10,13,14} from the original fixture
## ------------------------------------------------------------
...
## Utilities
is_spd <- function(M, tol = 1e-12) {
  if (!isSymmetric(M)) return(FALSE)
  ev <- suppressWarnings(eigen(M, symmetric = TRUE, only.values = TRUE)$values)
  all(ev > tol)
}
...
td[[8]] <- list(
  i = 14L,
  Sigma = matrix(c(
    59.227, 2.601, 3.38, 8.303, -0.334, 11.029, 10.908, 0.739, 4.703, 7.075, 8.049, 1.403, 9.838, 5.46, 11.949, 2.272, 7.234, 15.215, -9.091, 12.265, 3.01, -3.199, 10.608, 8.464, -8.685,
    2.601, 77.213, 0.882, 2.99, -2.536, -4.55, -3.874, -3.607, 6.023, 3.129, 15.7, -7.271, -10.655, 8.456, 15.387, -5.764, 6.617, -6.331, -2.244, -0.925, -7.516, 13.836, 9.243, -0.84, -3.781,
    3.38, 0.882, 79.72, -1.465, 3.179, -1.799, 9.842, 9.165, -1.54, -8.03, 0.778, 9.053, -2.598, -8.844, 15.857, 13.613, -1.878, 8.18, 11.806, -0.242, 4.711, 2.258, 9.554, 1.184, -8.047,
    8.303, 2.99, -1.465, 65.041, 3.803, 2.13, -0.936, -5.996, 2.719, -4.648, 4.611, 4.486, 13.38, -0.376, 0.179, 14.654, -7.089, -1.194, 9.357, 5.12, 4.943, -0.475, 4.764, -8.56, 2.337,
    -0.334, -2.536, 3.179, 3.803, 64.128, 4.85, -14.767, -10.044, 12.437, 5.065, 8.191, 0.391, 0.068, 9.754, -0.062, 0.429, 9.265, 5.502, 4.227, 0.559, 0.811, 3.169, 4.558, -1.878, -3.885,
...
## --------------------------
## Evaluation by case and df ν
## --------------------------
dfs <- c(2, 5, 10, 20)

evaluate_case <- function(obj, nu, repair_if_needed = TRUE, tol_pd = 1e-10) {
  S <- obj$Sigma
  b <- obj$b
  d <- length(b)
  ...
  if (!isSymmetric(S)) {
    status <- "fail"
    note <- "Sigma not symmetric"
    return(data.frame(i=obj$i, d=d, nu=nu, status=status, repaired=repaired, p=NA_real_, note=note))
  }
  
  # Correlation
  R <- cov2cor_mat(S)
  
  # Is R a valid correlation?
  R_is_spd <- is_spd(R, tol = tol_pd)
  ...
  if (!R_is_spd) {
    if (!repair_if_needed) {
      ...
      # Projection to nearest PD correlation
      R_used <- as.matrix(nearPD(R, corr = TRUE)$mat)
      repaired <- TRUE
      note <- "R repaired by nearPD"
    }
  }
  
  # t-Copula parameters (rho vector from lower triangle)
  rho <- R_used[lower.tri(R_used)]
  
  # u = marginal t-CDF at upper limits b
  u <- pt(b, df = nu)
  
  # t-Copula and probability
  p <- NA_real_
  status <- if (repaired) "ok_repaired" else "ok"
  
  # Direct attempt; if it fails, try to force a slight additional projection
  out <- try({
    C <- tCopula(param = rho, df = nu, dim = d, dispstr = "un")
    p <- pCopula(u, C)
  }, silent = TRUE)
  
  if (inherits(out, "try-error")) {
    if (!repaired && repair_if_needed) {
      # A second attempt: repair even if R was SPD (numerically borderline cases)
      R2 <- as.matrix(nearPD(R_used, corr = TRUE)$mat)
      rho2 <- R2[lower.tri(R2)]
      repaired <- TRUE
      status <- "ok_repaired"
      note <- if (is.na(note)) "Construct error; repaired by nearPD" else paste(note, "| construct error repaired")
      out2 <- try({
        C <- tCopula(param = rho2, df = nu, dim = d, dispstr = "un")
        p <- pCopula(u, C)
      }, silent = TRUE)
      if (inherits(out2, "try-error")) {
        status <- "fail"
        note <- paste("Construct error even after nearPD:", as.character(out2))
        p <- NA_real_
      }
    } else {
      status <- "fail"
      note <- paste("Construct error:", as.character(out))
      p <- NA_real_
    }
  }
  
  data.frame(i=obj$i, d=d, nu=nu, status=status, repaired=repaired, p=p, note=note)
}

## Run for all cases and dfs
results <- do.call(rbind, lapply(td, function(obj) {
  do.call(rbind, lapply(dfs, function(nu) evaluate_case(obj, nu)))
}))

## Order by case and df
results <- results[order(results$i, results$nu), ]

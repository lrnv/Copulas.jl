...
The pilot Pickands estimator is one of `:ols`, `:cfg`, or `:pickands`. The
multivariate CFG/OLS construction follows Gudendorf and Segers
[gudendorf2011nonparametric](@cite).

For `d ≥ 3`, ordinary convexification is not sufficient to characterize a valid
Pickands dependence function. The pilot is therefore projected by least squares
onto a class generated by a finite spectral measure supported on a simplex
grid, following the shape-constrained projection methodology of Gudendorf and
Segers [gudendorf2012multivariate](@cite).

If the spectral grid has atoms `v₁,…,vₘ` and masses `h₁,…,hₘ`, the projected
Pickands function is

```math
A(w)=\\sum_{k=1}^m h_k\\max_i(w_i v_{ik}),
```

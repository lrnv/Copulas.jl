...
Construct the `d`-dimensional Archimedean copula generated by `G`. The
constructor requires `d ≤ max_monotony(G)`. Its CDF is

```math
C(\\mathbf u) = \\phi\\left(\\sum_{i=1}^d \\phi^{-1}(u_i)\\right)
```

The generic sampler uses the radial--simplex representation obtained from the
inverse Williamson transform of ``\\phi``. A working CDF requires an evaluable
generator inverse; density and sampling additionally require the derivatives
and radial law described by the generator's documented capabilities. A
generator being valid in dimension `d` does not by itself guarantee that every
numerical operation is available or stable.

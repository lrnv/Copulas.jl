...
TCopula{d}(ν::Real, Σ::AbstractMatrix) where {d} = _promoted_t_copula(Val(d), ν, Σ)
# Resolve the intersection between the broad promotion constructor above and
# the exact `(T, Matrix{T})` constructor generated by `@paramorph`.
TCopula{d}(ν::Real, Σ::Matrix{T}) where {d,T} = _promoted_t_copula(Val(d), ν, Σ)

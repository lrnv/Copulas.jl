...
# Own the exact matrix signature generated by `@paramorph` so public matrix
# inputs are normalized before the constrained inner constructor is reached.
GaussianCopula{d}(Σ::Matrix{T}) where {d,T} =
    _promoted_gaussian_copula(Val(d), Σ)

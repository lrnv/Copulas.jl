...
Construct an Archimax copula by combining Archimedean generator `gen` with
stable tail dependence function `tail`. Let ``x_i=\\phi^{-1}(u_i)``; its CDF is

```math
C(\\mathbf u) = \\phi\\bigl(\\ell(x_1,\\ldots,x_d)\\bigr).
```

Reductions
- If ``\\ell(x)=\\sum_i x_i``, this is the Archimedean copula generated by `gen`.
- If ``\\phi(s)=e^{-s}``, obtained with `IndependentGenerator()`, this is the
  extreme-value copula defined by `tail`.

`params(::ArchimaxCopula)` concatenates the parameter tuples of `gen` and `tail`.
Both components must be valid in `d` dimensions. Smooth components generally
use the generic density and bivariate sampling machinery; singular radial or
spectral components retain their non-Lebesgue measure semantics. Generic
fitting is currently limited to supported typed component combinations.
...
See also: [`Generator`](@ref), [`Tail`](@ref),
[`ArchimedeanCopula`](@ref), [`ExtremeValueCopula`](@ref),
